		...
		if imena[n][k] !=0:
			...
			for j in range(1,3):
				...
				for l in range (0,24):
					...
					#limit=[]
					for i in range(0,3): 
						path = '/media/sf_Documents/diplomski/podaci_CLM1/{}/{}_{}_{}/{}/'.format(gcm[i],var[k],exp[j],exp[0],GRAD[l])
						if i == 0:
						...
						np.save('/media/sf_Documents/diplomski/rezultat_tmean_{}_{}_{}_{}_{}_clm'.format(exp[j],GRAD[l],gcm[i],imena[n][k],indeks[n]), data.tolist())
			np.save('/media/sf_Documents/diplomski/sred_tmean_{}_{}_clm'.format(imena[n][k],indeks[n]), sred)
dat.to_excel("/media/sf_Documents/diplomski/tmean_clm.xlsx")
...
for n in range(0,len(indeks)):
	for k in range(0,len(imena[n])):
		if n==0:
			...
			var=['tas','tas','tas']
		if imena[n][k] !=0:		
			sred=np.load('/media/sf_Documents/diplomski/sred_tmean_{}_{}_clm.npy'.format(imena[n][k],indeks[n]))
			print(type(sred))
			for l in range (0,24):	
				plt.close('all')
				fig, axs = plt.subplots(3, 1,figsize=(12, 36))
				fig.suptitle('{} {} CLM'.format(GRAD[l],naslov[n][k].replace('_', ' ')),fontsize=30, wrap=True)
				time=np.arange(1971,2071)
				limit= []
				for i in range(0,3): 
					for j in range(1,3):
						rezultat=np.load('/media/sf_Documents/diplomski/rezultat_tmean_{}_{}_{}_{}_{}_clm.npy'.format(exp[j],GRAD[l],gcm[i],imena[n][k],indeks[n]))
						print('/media/sf_Documents/diplomski/rezultat_tmean_{}_{}_{}_{}_{}_clm.npy'.format(exp[j],GRAD[l],gcm[i],imena[n][k],indeks[n]))
						

						if n==0:
							celz_rez = rezultat -273.15
							celz_sred = sred[j-1][l][i] -273.15
							axs[i].plot(time,celz_rez , label='{}'.format(exp[j]), color = '{}'.format(shade[j-1]))
							if j == 1:
							...
							axs[i].plot(time,rezultat, label='{}'.format(exp[j]),  color = '{}'.format(shade[j-1]))
							if j==1:
				...
				for i in range(0,3): 
					...
					path2 = '/media/sf_Documents/diplomski/tmean_CLM/'
					if not os.path.exists(path2):
						os.makedirs(path2)

					plt.savefig('{}{}_{}_tmean_CLM.png'.format(path2,GRAD[l],imena[n][k]))					
