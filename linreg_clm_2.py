	...
	for k in range(0,len(imena[n])):
		...
		if imena[n][k] !=0:
			...
			for j in range(1,3):
				...
				fig.suptitle('{} {} - CLM'.format(naslov[n][k].replace('_', ' '), exp[j]),fontsize=30, wrap=True)
				...
				for i in range(0,3):
					if rezultat.any() != 0: 
						...
						x,y=m(LON,LAT)
						z = rezultat[j-1][i]
						graf=axs[i].scatter(x, y,s=500, c=z, alpha=0.6,cmap='autumn_r')
						cb=plt.colorbar(graf,fraction=0.03, ax=axs[i],format = "%0.1f")
						cb.set_label(label='{} \n stopa porasta po 10 godina '.format(naslov[n][k].replace('_', ' ').capitalize() ), size=15)
						cb.ax.tick_params(labelsize='large')
						cb.mappable.set_clim(rezultat.min() , rezultat.max() ) 
						for m, label in enumerate(GRAD):
	    						axs[i].annotate(label, (x[m], y[m]),fontsize=15)
				...
				path2 = '/home/klara/Documents/diplomski/linreg_CLM/'
				if not os.path.exists(path2):
					os.makedirs(path2)

				plt.savefig('{}{}_{}_slope_CLM.png'.format(path2,imena[n][k], exp[j]))
					
dat.to_excel("/home/klara/Documents/diplomski/linreg.xlsx") 					
