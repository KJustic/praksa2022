...
for l in range (0,21):
	...
	for i in range(0,4):
		...
		axs[i].set_ylabel('heat wave duration index')
		axs1[i].set_ylabel('heat waves')
		axs[i].set_title('{}'.format(gcm[i]),fontsize=20)
		axs[i].legend()
		axs1[i].set_title('{}'.format(gcm[i]),fontsize=20)
		axs1[i].legend()

	path3 = '/home/klara/Documents/praksa/eca_hwdi/'
	if not os.path.exists(path3):
		os.makedirs(path3)
	fig.savefig('{}{}_heat_wave_duration_index.png'.format(path3,GRAD[l]))
	fig1.savefig('{}{}_heat_waves.png'.format(path3,GRAD[l]))
